#!/usr/bin/env bun
/**
 * hodlmm-idle-liquidity-recover — Recover out-of-range idle HODLMM liquidity with one explicit move.
 *
 ...
 * One operator-confirmed rescue transaction via move-relative-liquidity-multi.
 *
 * Commands:
...
const STATE_FILE = path.join(os.homedir(), ".hodlmm-idle-liquidity-recover-state.json");
...
function log(...args: unknown[]): void {
  process.stderr.write(`[idle-liquidity-recover] ${args.join(" ")}\n`);
}
...
program.name("hodlmm-idle-liquidity-recover").description("HODLMM Move-Liquidity & Auto-Rebalancer");
